...
def addFile(path):
    xml_root = read_xml("tools.config")
    # 2. 属性修改
    # A. 找到父节点
    file_nodes = find_nodes(xml_root, "Config/Files")
    # <file md5="45c059bd6277c9a986996f8bd76fdc41" path="Tools\KvfwScan\KvfwScan.exe" size="2122752" time="1586329369918" />
    a = create_node("file", {"md5": "45c059bd6277c9a986996f8bd76fdc41", "path": path, "size": "2122752",
                             "time": "1586329369918"}, "\n")
    add_child_node(file_nodes, a)
    write_xml(xml_root, "tools.config")
...
if __name__ == "__main__":
    in_path = read_conf("c.txt")
    for iter_path in in_path:
        print(iter_path)
        addFile(iter_path)
